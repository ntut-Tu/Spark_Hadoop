...
from configs.feature_select import get_default_score_features, get_testing_score_features, get_score_features
...
def run(df, config):
    assembler = VectorAssembler(inputCols=_mode_helper("new"), outputCol="score_vector")
...
def predict_with_score_model(df, config):
    ...
    assembler = VectorAssembler(inputCols=get_default_score_features(), outputCol="score_vector")
    df = assembler.transform(df)
